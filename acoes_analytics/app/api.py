"""
QuantView — FastAPI Backend
=============================
API REST para análise quantitativa de portfólios.
"""
...
from app.schemas import (
    ...
    CAPMResponse, CorrResponse, RiskResponse, VaRResponse, ValuationResponse,
)
from app.core import (
    ...
    var_parametric, stress_scenarios, fetch_valuation_multiples,
)

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger("quantview")

app = FastAPI(
    title="QuantView API",
    description="Quantitative Portfolio Analytics & Risk Assessment",
    version="2.0.0",
)

# CORS — permite chamadas do Streamlit Cloud
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
@app.get("/")
def root():
    return {"status": "ok", "service": "quantview-api", "version": "2.0.0"}
...
def markowitz(req: AnalysisRequest):
    ...
    # PORT do usuário (calculado no backend com mesmos mu/cov)
    ...
    return {
        ...
        "min_variance": {
            ...
            "ret": r_mv, "vol": v_mv, "sharpe": s_mv,
        },
        "port_user": {
            "weights": dict(zip(req.tickers, w_user.tolist())),
            "ret": r_user, "vol": v_user, "sharpe": s_user,
        },
        "inputs": {
            "tickers": req.tickers,
            "rf_annual": req.rf_annual,
...
# =============================
# VaR & Stress Testing (NOVO)
# =============================

@app.post("/var", response_model=VaRResponse)
def var_endpoint(req: AnalysisRequest):
    """Value at Risk (95% e 99%) + cenários de stress do portfólio."""
    try:
        prices = fetch_adj_close(req.tickers, req.start, req.end)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    ...
    ret_simple, _ = compute_daily_returns(prices)
    w = normalize_weights(req.weights, len(req.tickers))
    port = portfolio_returns(ret_simple, w)

    var95 = var_parametric(port, confidence=0.95, trading_days=req.trading_days)
    var99 = var_parametric(port, confidence=0.99, trading_days=req.trading_days)
    scenarios = stress_scenarios(ret_simple, w, trading_days=req.trading_days)
...
# =============================
# Valuation Multiples (NOVO)
# =============================

@app.post("/valuation", response_model=ValuationResponse)
def valuation(req: AnalysisRequest):
    """Múltiplos de valuation (EV/EBITDA, P/E, P/B, etc.) via yfinance."""
    try:
        df = fetch_valuation_multiples(req.tickers)
    except Exception as e:
        logger.error(f"Erro ao buscar múltiplos: {e}")
        raise HTTPException(status_code=500, detail="Erro ao buscar dados de valuation.")

    return {"multiples": df.to_dict()}
