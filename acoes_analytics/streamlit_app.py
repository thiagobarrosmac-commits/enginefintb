...
with col1:
    tickers_raw = st.text_input("Tickers (separados por vírgula)", value="PETR4.SA,VALE3.SA")
with col2:
...
with col5:
    n_portfolios = st.number_input("N carteiras (Markowitz)", min_value=1000, max_value=50000, value=8000, step=1000)
with col6:
...
if run:
    ...
    annual = pd.DataFrame(ret["annual_returns"])

    c1, c2 = st.columns(2)
    with c1:
        st.markdown("**Dataset: Retorno diário (simples)**")
        st.dataframe(daily_simple.tail(20))
    ...
    with c2:
        ...
        st.dataframe(daily_log.tail(20))
        fig = px.line(daily_log, title="Retorno diário (log)")
        st.plotly_chart(fig, use_container_width=True)

    st.markdown("**Dataset: Estatísticas anualizadas (retorno e volatilidade)**")
    st.dataframe(annual)
    fig = px.bar(annual.T, barmode="group", title="Retorno e Volatilidade anual (por ticker)")
    st.plotly_chart(fig, use_container_width=True)

    st.divider()
    st.subheader("3) Sharpe")
    sh = call("/sharpe", payload)
    sharpe_df = pd.DataFrame(sh["sharpe_annual"])
    stats_df = pd.DataFrame(sh["stats_annual"])

    c3, c4 = st.columns(2)
    with c3:
        st.markdown("**Dataset: Sharpe anual**")
        st.dataframe(sharpe_df)
        fig = px.bar(sharpe_df.T, title="Sharpe anual")
        st.plotly_chart(fig, use_container_width=True)

    with c4:
        st.markdown("**Dataset: Stats anual (ret/vol)**")
        st.dataframe(stats_df)
        fig = px.scatter(stats_df, x="vol_annual", y="ret_annual", title="Risco x Retorno (anual)", text=stats_df.index)
        st.plotly_chart(fig, use_container_width=True)
    ...
    if len(tickers) < 2:
        ...
        with c5:
            fig = px.scatter(pts, x="vol", y="ret", title="Simulação Markowitz (vol x ret)", hover_data=["sharpe"])
            fig.add_trace(go.Scatter(x=[max_sh["vol"]], y=[max_sh["ret"]], mode="markers", name="Max Sharpe"))
            fig.add_trace(go.Scatter(x=[min_v["vol"]], y=[pts.loc[pts["vol"].idxmin(), "ret"]], mode="markers", name="Min Variância"))
            st.plotly_chart(fig, use_container_width=True)
        with c6:
